...
from lv_functions import A_matrix, A_matrix_upd
...
# region set matrices 
A = A_matrix_upd(n, C, sigma2, seed, LH=1)
...
zvec = np.zeros(s)
for i in range(s):
    cc = M[2*i][2*i]; aa = M[2*i+1][2*i+1]; ff = M[2*i][2*i+1]; gg = M[2*i+1][2*i]
    z = (cc-aa+((cc-aa)**2 +4*gg*ff)**0.5)/(2*gg)
    R = (aa + z*gg)/(1+z)
    zvec[i] = z
    Rvec[i] = R
...
Ainv_rs = np.dot(A_inv, np.ones(s))
#print('Ainv rs: \n', Ainv_rs)
...
for i in range(s):
    xf_an[2*i] = zvec[i]*xf_an[2*i]
print('z(0): ', zvec[0], ', rat:', xf_an[0]/xf_an[1])
