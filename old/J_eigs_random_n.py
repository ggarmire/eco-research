...
ns = [4, 4, 4, 4, 6, 6, 6, 6, 8, 8, 8, 8, 10, 10, 10, 10, 14, 14, 14, 14, 16, 16, 16, 16, 18, 18, 18, 18, 20, 20, 20, 20, 24, 24, 28,28]
# to change
sigma2 = 0.0064
...
# constants for use 
fs = np.random.uniform(0.6, 2, len(ns))
gs = np.random.uniform(0.6, 2, len(ns))
mucs = np.random.uniform(-0.5, 0, len(ns))
muas = np.random.uniform(-0.5, 0, len(ns))
...
plt.figure()
plt.plot(ns, Jsigma2s, '.', label = 'sigma')
plt.xlabel('n')
plt.ylabel('sigma2 of J')
plt.grid()

plt.figure()
plt.title('Eigs of M vs. J, varied n')
...
plt.figure()
plt.title('Sigma^2 M vs. J, varied n')
