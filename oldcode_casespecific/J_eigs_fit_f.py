...
df = pd.DataFrame({'f': fs, 'mean': means, 'sigma2': sigma2s})

df.to_csv('J_fit_data_f.csv', index = False)

plt.figure()
plt.plot(fs, means, label = 'K')
plt.xlabel('f')
