...
for run in range(runs):
    if n > 50 and run%20 ==0:
        print(run)
    K = Ks[run]
    sigma2 = K**2/n      # sigma to give complexity K in the classical case 
    ...
    if seed != 0:
        ...
        num = run
        #print(num)
        A = A_matrix(n, C, sigma2, run, LH=0)
...
mss = 4
if seed != 0:   
    plot_title = str('Max real eigenvalue for K: '+str(n)+' species, A seed = ' +str(seed)+', no LH')
else:
    plot_title = str('Max real eigenvalue for K: '+str(n)+' species, random As, no LH')
