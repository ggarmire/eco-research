...
from functions.lv_functions import A_matrix, M_matrix, LH_jacobian, x0_vec
import random 
import math

def lv_LH_quad(x0, t, A, M, omega): 
    def derivative(x, t, M, A):
        omegax = np.dot(omega, x)
        Mx = np.dot(M, x)
        for i in range(len(x0)):
            if x[i] <= 1e-5:
                x[i] = 0
        dxdt = Mx + np.multiply(x, np.dot(A, x))
        for i in range(len(x0)):
            if i%2 == 0:    # child
                if M[i][i+1] + omegax[i] > 0:
                    dxdt[i] += x[i+1]*omegax[i]
                else: dxdt[i] -= M[i][i+1]*x[i+1]
            elif i%2 == 1:
                if M[i][i-1] + omegax[i] > 0:
                    dxdt[i] += x[i-1]*omegax[i]
                else: dxdt[i] -= M[i][i-1]*x[i-1]
...
seed = 434 # 432 stable 644 unstable 
print('\n\n')
print("seed: ", seed)
...
omega = 0.1
omegamat = omega*np.random.normal(1, 0.2, (n,n))

z = (muc-mua+((muc-mua)**2 +4*g*f)**0.5)/(2*g)
R_c = muc+f/z; R_a = z*g+mua
...
# region numerical
result = lv_LH_quad(x0, t, A, M, omegamat)
xf_num = result[-1,:]
z_num = np.divide(result[:,::2], result[:, 1::2])
Z_num = np.divide(result[:,::2], (result[:, 1::2]+result[:, ::2]))
#endregion
for i in range (n):
    print('std of x[i]: ', np.std(result[-20:-1, i]))




A_scaled = np.dot(np.diag(xf_num), A)
...
print('Mp eig: ', min(Mpvals[1], Mpvals[0]), ', from M: ', -abs(mvals[0]-mvals[1]))
...
mpar_text = str('$\u03bc_c =$'+str(muc)+', $\u03bc_a =$'+str(mua)+', $f=$'+str(f)+', $g =$'+str(g)+' (z='+str('%.2f'%z)+'), $\omega=$'+str(omega))
apar_text = str('n='+ str(n)+', A seed ='+ str(seed)+', K='+str(K_set))



plot_text2 = str('Max real eig J (analytical): '+ str('%.3f'%(np.max(np.real(Jvals)))) +"\nM' eigenvalue: "+str('%.3f'%(-abs(mvals[0]-mvals[1])))) #+', R='+str('%.3f'%R_a)))
...
plt.plot(xf_an[::2], xf_num[::2], 'o', label = 'child populations')
plt.plot(xf_an[1::2], xf_num[1::2], 'o', label = 'adult populations')
...
plt.figure()
plt.grid()
plt.plot(z_num[-1,:], zvec, '.')
plt.plot(np.linspace(0, 2, 5), np.linspace(0, 2, 5))
plt.title('z: analytical vs numerical')'''
