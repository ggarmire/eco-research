...
f = 1.5
g = 1
mucs = np.linspace(-1.4, -0.1, length)
mua = -0.5
...
df = pd.DataFrame({'muc': mucs, 'mean': means, 'sigma2': sigma2s})

df.to_csv('J_fit_data_muc.csv', index = False)
