...
f = 1.5
gs = np.linspace(0.55, 1.5, length)
muc = -0.5
mua = -0.5
...
df = pd.DataFrame({'g': gs, 'mean': means, 'sigma2': sigma2s})

df.to_csv('J_fit_data_g.csv', index = False)

plt.figure()
plt.plot(gs, means, label = 'K')
plt.xlabel('g')
