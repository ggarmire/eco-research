...
length = 50
# to change
K_sets = np.linspace(0.01, 2, length)
...
for i in range(length):
    ...
    sigma2_sets.append(sigma2)


    M_pre = M_matrix(n, muc, mua, f, g)

    if i%5 == 0: print('on loop ', i, 'f/g/muc/mua/K: ', f, g, muc, mua, K)
...
df = pd.DataFrame({'K': K_sets, 'sigma2_set': sigma2_sets, 'mean': means, 'meanerr': meanerrs, 'sigma2': sigma2s})

df.to_csv('J_fit_data_K.csv', index = False)

plt.figure()
plt.plot(K_sets, means, label = 'K')
plt.plot(sigma2_sets, means, label = 'sigma2 set')
plt.xlabel('K')
...
plt.figure()
plt.plot(K_sets, sigma2s, label = 'K')
plt.plot(sigma2_sets, sigma2s, label = 'sigma2 set')
plt.xlabel('K')
